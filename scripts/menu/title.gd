...
func _on_b_play_pressed() -> void:
	...
	for player_id in NetworkManager.players.keys():
		if !NetworkManager.players[player_id]["Ready"]:
			all_ready = false
			break
	if all_ready:
...
func _change_player_name(new_name: String) -> void:
	...
	if NetworkManager.players.has(my_peer_id):  # Check if the key exists
		NetworkManager.players[my_peer_id]["Name"] = new_name
		NetworkManager._sync_player_info.rpc(NetworkManager.players)  # Sync player info
		_show_player_name()
		_show_player_list.rpc()
	else:
		print("Error: Player ID ", my_peer_id, " not found in NetworkManager.players.")
...
func _show_player_name() -> void:
	...
	if NetworkManager.players.has(my_peer):
		player_name.text = str(NetworkManager.players[my_peer]["Name"])
	else:
		print("Error: Player ID ", my_peer, " not found in NetworkManager.players.")
...
func _server_disconnected() -> void:
	print("Server disconnected. Clearing players.")
...
#Character
func _change_character(peer_id: int, part: String, frame: int) -> void:
	if multiplayer.is_server():
...
func _on_b_left_head_pressed() -> void:
	...
	_change_character(multiplayer.get_unique_id(), "Head", head_sprite.frame)
...
func _on_b_left_face_pressed() -> void:
	...
	_change_character(multiplayer.get_unique_id(), "Face", face_sprite.frame)
